...
**Phase 1の補足：既存仕様書からの形式化——MD→VDM-SL→MDパイプライン**

前述のPhase 1対話例は「ゼロから仕様を策定する」ケースだった。しかし実際の開発現場では、すでにMarkdown等で書かれた不完全な仕様書（要件定義書、設計ドキュメント、API仕様書など）が存在していることが多い。この場合、ゼロから対話を始めるのではなく、既存文書を出発点として形式化を進める方が効率的だ。

このアプローチの核心は、**自然言語をVDM-SLに変換しようとする行為そのものが、仕様の検証行為として機能する**という点にある。VDM-SLの型定義や事前条件・事後条件を書こうとしたとき、「書けない箇所」が構造的に浮かび上がる。書けない理由は、元の自然言語仕様が曖昧だからだ。

具体的なプロセスを、ECサイトの注文処理仕様書を例に示す。

*ステップ1：文書の構造解析と要件分類*

AIはMarkdown文書を読み込み、構造要素を自動的に分類する。見出しはモジュールやエージェントの境界候補に、箇条書きは要件・制約・ルールに、テーブルはデータモデルに、番号付きリストは逐次操作やワークフローに対応する。各要件は「VDM-SLのどの構成要素に写像されるか」に分類される。「商品にはIDと名前と在庫数がある」は型定義（`types`）に、「在庫は0未満にならない」は不変条件（`inv`）に、「注文を作成できる」は操作（`operations`）に写像される。

*ステップ2：VDM-SLへの変換試行による曖昧性の体系的検出*

ここが核心だ。自然言語をVDM-SLに変換しようとすると、7つのパターンの曖昧性が構造的に露呈する。

第一に、曖昧な数量詞。仕様書に「ユーザーは複数のアドレスを持てる」と書かれていた場合、VDM-SLでは `seq of Address`（制限なし）か `seq1 of Address`（1つ以上必須）か `inv addrs == len addrs <= N`（上限N個）かを選ばなければ型が定義できない。「複数」という自然言語では、この区別が曖昧なまま放置されている。

第二に、未定義の用語。「アクティブなユーザーのみ注文可能」と書かれているが、「アクティブ」の定義がない。VDM-SLで合併型 `<ACTIVE> | <SUSPENDED> | <DELETED>` を定義するには、取りうる状態をすべて列挙する必要がある。

第三に、暗黙の制約。「ユーザーは注文できる」としか書かれていない場合、VDM-SLの事前条件を書こうとした時点で「他のユーザーの代理注文は可能か」「同時注文数の上限はあるか」という問いが自然に発生する。

第四に、欠落したエラーケース。事後条件を書く際に「支払いが失敗したら注文状態はどうなるか」「在庫切れの場合は」が未定義であることが判明する。ハッピーパスのみの記述では事後条件が書けない。

第五に、曖昧な関係。「商品はカテゴリに関連する」と書かれている場合、VDM-SLでは `map ProductId to CategoryId`（多対1）か `map ProductId to set of CategoryId`（多対多）かを決めなければならない。

第六に、時間的曖昧性。「支払い後に在庫を引き当てる」の「後」は厳密な順序か。並行実行は許容するか。操作の事前条件・事後条件の連鎖として表現するには、この順序関係を明示する必要がある。

第七に、境界条件。「在庫数は最大1000個まで」と書かれている場合、不変条件を `<= 1000` と書くか `< 1000` と書くかで意味が変わる。

*ステップ3：優先度付き対話による曖昧性の解消*

検出された曖昧性は、3つの優先度に分類されてユーザーに提示される。「ブロッキング」はこれが決まらないとVDM-SLが一行も書けないもの（例：状態機械の状態一覧）。「重要」は事前条件・事後条件・不変条件に影響するもの（例：境界値の包含・排他）。「明確化」は仕様をより精密にするが、なくても進められるもの（例：リストの上限値）だ。

関連する質問はグループ化して対話のラウンド数を最小化する。複雑な文書では3〜5ラウンドの対話が必要になることもある。この対話プロセスは、第5.3節で論じた「形式記法は思考の規律として機能する」の具体的発現だ。

重要な原則として、**AIが曖昧性を暗黙的に解決することは決してない**。すべての解釈の選択がユーザーに提示される。形式化の全目的は暗黙のものを明示的にすることにあるため、この原則は非妥協的だ。

*ステップ4：トレーサビリティ付きVDM-SL仕様の生成*

対話で解消された情報をもとにVDM-SL仕様を生成する。ここでの鍵は、双方向トレーサビリティだ。各VDM-SL要素に `[REQ-nnn]` タグと元の自然言語テキストをコメントとして付与する。

```vdm
operations
  -- [REQ-004] 「ユーザーは注文を作成できる」
  CreateOrder(userId: UserId, items: seq1 of OrderItem) res: OrderId
    ...
    pre userId in set dom users
        and users(userId).status = <ACTIVE>    -- 対話で確定: アクティブユーザーのみ
        and len items <= 50                    -- 対話で確定: 1注文あたり最大50品目
    post res in set dom orders
        and orders(res).userId = userId;
```

このコメント構造が、後のステップで自然言語仕様書を再生成する際のソース情報になる。

生成後、ギャップ分析が行われる。「元文書の要件のうちVDM-SLに写像されたもの」「写像できなかったもの（UI要件、パフォーマンス要件などの非機能要件）」「AIが推論で補完したもの」が明示的に区別される。推論で補完された要素は、必ずユーザーの確認を経る。

*ステップ5：機械的検証*

生成されたVDM-SL仕様をVDMJ等のツールで機械的に検証する。構文チェック、型チェック、証明責務（PO）の生成が自動実行され、結果は自然言語で解説される。たとえば「CreateOrderがOrderId型の値を返すとき、その値がordersのキーに含まれることが保証されていますか？」という形で、形式手法の知識がなくても理解できるように提示される。

ここで見つかった問題は、ステップ3の対話に戻って解消される。このループが、仕様の内部一貫性を担保する。

*ステップ6：VDM-SLから自然言語仕様書への逆変換*

検証済みのVDM-SL仕様を、ドメインエキスパートや非技術者がレビューできる自然言語仕様書（Markdown）に逆変換する。変換規則は忠実な翻訳であり、簡略化ではない。型定義は「何が存在するか」に、状態定義は「システムが何を記憶するか」に、事前条件は「前提条件——操作実行前に真でなければならないこと」に、事後条件は「保証——操作成功後に真になること」に翻訳される。

生成される仕様書は、概要・用語集・データモデル・操作・ビジネスルール・制約と制限事項という構造を持つ。各要素はVDM-SLソースへのトレーサビリティを保持する。品質チェックとして、VDM-SL中の全要素が文書に現れているか（完全性）、自然言語がVDM-SLの意味を正確に伝えているか（忠実性）、形式的記法が漏れていないか（可読性）、用語が一貫しているか（一貫性）が検証される。

重要な点として、**この自然言語仕様書は派生成果物であり、VDM-SLが正（source of truth）である**。文書ヘッダにその旨が明記される。これにより、ステークホルダーのレビューを経た修正要望は再びVDM-SLへの反映→検証→再生成というサイクルを通ることになり、仕様の厳密性が維持される。

このMD→VDM-SL→MDパイプラインの全体像は次の通りだ。

```
不完全なMD仕様書
    ↓ 構造解析・要件分類
    ↓ VDM-SL変換試行 → 曖昧性の体系的検出
    ↓ 優先度付き対話（3〜5ラウンド）
    ↓ トレーサビリティ付きVDM-SL生成
    ↓ 機械的検証（VDMJ）
    ↓ 自然言語仕様書（MD）への逆変換
検証済み自然言語仕様書（VDM-SLが正）
    ↓ ステークホルダーレビュー
    ↓ 修正要望 → VDM-SLに反映（サイクル再開）
```

Phase 1の対話がイテレーティブであることは先に述べたが、このパイプラインはその具体的な仕組みを提供する。ドメインエキスパートの暗黙知は初回の対話で100%引き出せるものではなく、実際にシステムを使って初めて気づくルールがある。自然言語仕様書を介したレビューサイクルにより、暗黙知は段階的に形式化されていく。
...
**非機能要件の対話的引き出し：** ここで実務上の問題がある。「レスポンスタイム200ms以下」のような定量的な非機能要件を、ドメインエキスパートが最初から適切に設定できるとは限らない。200msが妥当か500msが妥当かは、ユーザー体験の知見とインフラコストのトレードオフであり、純粋なドメイン知識とは異なる領域だ。

この問題に対して、本手法ではAIが**体験を通じた引き出し**を行うことを想定する。たとえば、AIがモックUIを生成し、意図的に異なるレスポンス遅延（100ms、300ms、500ms、1000ms）を再現して人間に操作させる。人間は「この速度では遅すぎる」「これなら許容範囲だ」と体感で判断できる。あるいは、AIがインフラコストの概算を提示し、「200msを達成するにはこの構成で月額○○円、500msならこの構成で月額○○円です」と選択肢を示すことで、ドメインエキスパートがビジネス判断として意思決定できる。非機能要件の定量化もまた、Phase 1と同様にAIが提案し人間が判断するという対話構造の中で解決される。
...
**仕様中心の短サイクル——ウォーターフォールでもアジャイルでもない第三の構造**

ここで本手法のサイクル構造を、従来手法と対比して明確にしておく。

ウォーターフォールでは「要件定義→仕様策定→設計→コーディング→リリース→要件定義…」と一方向に流れる。アジャイルでは「要件定義→簡易設計→コーディング→リリース→要件定義…」と、リリースを挟んだフィードバックで改善を回す。いずれも、暗黙知が顕在化するのは**リリース後**——つまり動くシステムを人間が使った後だ。

本手法はこの構造が根本的に異なる。形式仕様が厳密であるからこそ、**リリースを待たずに仕様レベルでのテストとモック生成が可能**になる。サイクルは以下の通りだ。

> **要件の言語化 ↔ 形式仕様の策定 ↔ 仕様テスト・モック提示 → リリース**

まず人間が要件を伝え、AIがVDM-SL仕様を生成する。AIは仕様の内部整合性を検証し、矛盾や未定義のエッジケースを人間に提示する。さらに、仕様から自動生成されたモック（プロトタイプ）を人間に操作させることで、「使ってみて初めて気づく暗黙知」を**コードを一行も書く前に**引き出す。仕様のテストが論理的な矛盾を炙り出し、モックが体験的な気づきを炙り出す。この二重のフィードバックが仕様レベルで回るのだ。

建築に喩えれば、建てる前に3Dモデルで間取りを歩き回り、「この動線は使いにくい」「ここに棚が欲しい」と気づけるVR内見のようなものだ。実際に住んでからリフォームするより、はるかに効率的である。

このサイクルが十分に回った後に、Phase 2〜4をAIが自律的に実行し、仕様に適合するシステムをリリースする。もちろん、リリース後に新たな暗黙知が発見される可能性がゼロになるわけではない。しかし、仕様レベルでの論理的検証とモックによる体験的検証を経ているため、リリース後に浮上する暗黙知の量はアジャイル手法と比べて**大幅に削減**される。そして万一発見された場合も、同じ仕様中心サイクルに戻るだけだ。アジャイルのようにテストを書き直しリファクタリングの副作用に怯える必要はない——仕様を更新すればAIが新しいシステムを生成する。

**仕様を中心としたサイクルであること**——これが本手法の構造的な特徴である。
...
### 5.5 現段階での暫定的な運用——理想と今日の橋渡し

本手法のコンセプトは「AIエージェントの自律性が進化した数ヶ月先の未来からAGI到達までの間の、AI駆動開発の最適化」である。理想的なワークフロー（Phase 2〜4の完全自律サイクル）は、現在のLLMの能力では複雑なシステムに対して完全には実現できない。この事実を誠実に認めた上で、**現段階から本手法を段階的に導入するための暫定策**を以下に示す。

**Phase 2（設計）の暫定策：** 現在のLLMは、単純なCRUDシステムやAPIサービスの設計であれば自律的に行える。しかし、複雑な分散システムや高度なセキュリティ要件を持つシステムでは、経験豊富なアーキテクトの介入が必要になる場合がある。暫定策として、AIが設計案を生成し、人間のアーキテクトがレビュー・修正するハイブリッドモデルを採用する。重要なのは、このアーキテクトの介入を**過渡的なもの**と位置づけ、AIの自律性が向上するにつれて段階的に縮小していく前提で運用することだ。

**Phase 4（検証）の暫定策：** VDM-SLの事後条件からproperty-based testを自動生成し実行するサイクルは、現段階では単純な仕様に対してのみ完全に自律的に回せる。複雑な状態遷移や並行処理を含む仕様では、テスト戦略の設計にQAエンジニアの知見が必要になることがある。この場合も、QAエンジニアはテストケースを手書きするのではなく、AIが生成したテスト戦略のレビューと方針修正に徹する。テストの実行と結果判定は引き続きAIが自律的に行う。

**暫定策の共通原則：** いずれの暫定策においても、人間の介入は「AIの出力をレビューし、方針を修正する」という形態に限定される。人間がゼロからコードを書いたり、テストケースを手書きしたりすることはない。これにより、AIの自律性が向上した際に、人間の介入ポイントを自然に縮小できる。暫定策は本手法の理想像を否定するものではなく、理想への段階的な移行パスである。
