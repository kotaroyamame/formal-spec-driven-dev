...
このサイクルが十分に回った後に、Phase 2〜4をAIが自律的に実行し、仕様に適合するシステムをリリースする。もちろん、リリース後に新たな要件が発見されることもあるが、その場合も同じ仕様中心サイクルに戻るだけだ。アジャイルのようにテストを書き直しリファクタリングの副作用に怯える必要はない——仕様を更新すればAIが新しいシステムを生成する。
