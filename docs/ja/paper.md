...
**Phase 1へのフィードバックループ——イテレーティブな仕様進化**

ここで補足すべき重要な点がある。Phase 1→2→3→4は一方向のウォーターフォールではない。実際には、Phase 4で完成したシステムを人間が運用・操作することで、**当初は言語化できなかった暗黙知**が浮き彫りになる。「使ってみて初めて、ここにこういう機能が必要だと気づいた」という経験は、従来のソフトウェア開発でも日常的に発生する。

このとき、Phase 1に戻って仕様策定対話を再び行う。ただし、2回目以降のPhase 1は初回とは質的に異なる。人間は実際のシステムを触った経験を持っているため、要件をより具体的に言語化できる。AIも過去の対話履歴から、このドメインで見落としやすいエッジケースや暗黙の前提を学習しており、より鋭い質問を投げかけられる。建築に喩えれば、住んでみて初めて「ここに棚が欲しかった」「この動線は使いにくい」と気づき、次のリフォームではより具体的な要望を伝えられるようになるのと同じだ。

外部からはアジャイル方式を採用した開発チームのように見えるだろう。しかし本質的な違いがある。アジャイルではイテレーションごとにテストを書き直し、リファクタリングの副作用に怯えながらコードを修正する。本手法では、仕様を更新すればAIがPhase 2〜4を自律的に再実行し、仕様に適合する新しいシステムを生成する。イテレーションのコストが根本的に異なるのだ。
